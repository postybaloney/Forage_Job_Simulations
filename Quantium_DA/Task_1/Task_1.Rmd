...
customersbyPremium_Lifestage <- data |> group_by(LIFESTAGE, PREMIUM_CUSTOMER) |> summarise(count = n())
ggplot(customersbyPremium_Lifestage, aes(x = factor(PREMIUM_CUSTOMER), y = count, colour = factor(LIFESTAGE))) + geom_jitter(size = 3) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
```
...
avgUnitsbyPremium_Lifestage <- data |> group_by(PREMIUM_CUSTOMER, LIFESTAGE) |> summarise(avg_units = mean(PROD_QTY))
ggplot(avgUnitsbyPremium_Lifestage, aes(x = factor(LIFESTAGE), y = avg_units, colour = factor(PREMIUM_CUSTOMER))) + geom_jitter(size = 3) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
```
...
avgPricebyPremium_Lifestage <- data |> group_by(LIFESTAGE, PREMIUM_CUSTOMER) |> summarise(avg_price = mean(TOT_SALES/PROD_QTY))
ggplot(avgPricebyPremium_Lifestage, aes(x = factor(LIFESTAGE), y = avg_price, colour = factor(PREMIUM_CUSTOMER))) + geom_jitter(size = 3) + theme(axis.text.x = element_text(angle = 45, vjust = 0.5))
...
justMidandYoung <- avgPricebyPremium_Lifestage |> filter(LIFESTAGE == "MIDAGE SINGLE/COUPLES" | LIFESTAGE == "YOUNG SINGLES/COUPLES")
justMidandYoung$group <- ifelse(justMidandYoung$PREMIUM_CUSTOMER == "Mainstream", "Mainstream", "Premium_Budget")

t.test(avg_price ~ group, var.equal = TRUE, data = justMidandYoung)
```
The t-test results in a p-value of 0.01097, i.e. the unit price for mainstream, 
...
library(arules)
mainstream_young <- data |> filter((LIFESTAGE == "MIDAGE SINGLE/COUPLES" | LIFESTAGE == "YOUNG SINGLES/COUPLES") & PREMIUM_CUSTOMER == "Mainstream")
mainstream_young |> group_by(BRAND) |> summarise(count = n()) |> arrange(desc(count))

transactions <- as(split(mainstream_young$BRAND, seq(nrow(mainstream_young))), "transactions")
rules <- apriori(transactions, parameter = list(support = 0.1, confidence = 0.8))
head(rules)
```
We can see that: Kettle and Dorito chips were the most preferred chips
[INSIGHTS]

...
setwd("F:/Berkeley/Forage/Quantium_DA/Task_2")
filePath <- "F:/Berkeley/Forage/Quantium_DA/Task_2/"
...
data[, YEARMONTH := format(DATE, format = "%Y%m")]
...
score_nCustomers <- merge(corr_nCustomers, magnitude_nCustomers, by = c("Store1", "Store2"))[, scoreNCust :=
                                                                                               corr_weight*(corr_measure+mag_measure)]
```
...
ggplot(pastCustomers, aes(TransactionMonth, nCustomers, color = Store_type)) + geom_line() + labs(x = "Month of operation", y = "Total Customers", title = "Total Customers by Month")
```
## Assessment of trial
...
criticalValue <- qt(0.975, df = degreesOfFreedom)
percentageDiff[, isSignificant := tValue > criticalValue][, CriticalVal := criticalValue]
trialResults <- percentageDiff[YEARMONTH >= 201902 & YEARMONTH <= 201904, 
                                .(YEARMONTH, percentageDiff, tValue, isSignificant, CriticalVal)]
```
...
scaledControlCustomers <- measureOverTimeCusts[, 
                                        ][, controlCustomers := nCustomers * scalingFactorForControlSales
                                        ][, Store_type := "Control"]
percentageDiff <- merge(measureOverTimeCusts[STORE_NBR == trial_store, .(YEARMONTH, trialCusts = nCustomers, TransactionMonth)], 
                      scaledControlCustomers[, .(YEARMONTH, controlCustomers, TransactionMonth)], 
                      by = c("YEARMONTH", "TransactionMonth")
                      )[, percentageDiff := ((controlCustomers - trialCusts) / controlCustomers)]
```
...
geom_rect(data = trialAssessment[YEARMONTH > 201901 & YEARMONTH < 201905, ], aes(xmin = min(TransactionMonth), xmax = max(TransactionMonth),  
                                                                                 ymin =  min(nCusts), ymax = Inf, color = NULL), 
          show.legend = FALSE) +
geom_line() +
